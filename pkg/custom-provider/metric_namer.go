...
func NamersFromConfig(cfg *config.MetricsDiscoveryConfig, mapper apimeta.RESTMapper) ([]MetricNamer, error) {
	...
	for i, rule := range cfg.Rules {
		...
		if rule.Naming.Prefix != "" || rule.Naming.Suffix != "" {
			matcher, err := newReMatcher(config.RegexFilter{Is: rule.Naming.Prefix + ".*" + rule.Naming.Suffix})
			if err != nil {
				return nil, fmt.Errorf("unable to generate series name filter from naming associated with series query %q: %v", rule.SeriesQuery, err)
			}
